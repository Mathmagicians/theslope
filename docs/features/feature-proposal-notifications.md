...
| Prod env for the sender | `workers/sender/wrangler.toml` `[env.prod]` (routes `www` + apex `/sender/*`, consumer `theslope-sender-prod`, sender `no-reply@skraaningen.dk`), Makefile `deploy-sender-prod`, `logs-sender-prod`, `queues-info-prod`, `deploy-prod` = sender then app | ✅ 2026-09-18 wired; prod deploy with the merge to main |
...
- **Email provider**: `EmailProvider.send(EmailMessage) → {providerMessageId}` over `env.EMAIL.send({from: fromName ? {email: from, name: fromName} : from, to: toName ? {email, name} : to, replyTo, subject, text, html, attachments: [{content: bytes decoded from contentBase64 (the binding sends a string as the file's text), filename, type: contentType, disposition: 'attachment'}]})`.
...
- **Key scheme** (`server/utils/billingArchive.ts`, unit-tested): `getBillingArchiveKey(cutoffDate, version) → 'billing/2026-08/pbs-opgoerelse-2026-08-v1.csv'` (ASCII, from the cutoff month — `billingPeriod`'s `dd/MM/yyyy-dd/MM/yyyy` carries slashes); one object per version. The human filename `PBS-Opgørelse-Skråningen-<periode>-v<version>.csv` (`generateCsvFilename`) goes to `customMetadata.filename` and to the mail attachment.
...
- **`BillingArchiveResultSchema`** = `{key, filename, sizeBytes, version, archived, degraded}`; **`BillingPeriodSideEffectsSchema`** = `{billingPeriodSummaryId, billingPeriod, version, csvUploaded, emailSent, archive?, notification?}`; `MonthlyBillingResponseSchema += periods` (operation-result types, ADR-009).
- **`runMonthlyBilling(d1Client, triggeredBy, {queue, archive, notifications})`** (`server/utils/monthlyBillingService.ts`): after `generateBilling`, **every closed period converges** (user, 2026-09-17 — idempotency is the end state: transactions billed, CSV in R2, accountant mailed): `BillingPeriodSummary` carries `version` (domain: bumped by generation when catch-up changes a period); the delivery facts live in the generic append-only **`Delivery`** table (user, 2026-09-17: `subjectType` `BILLING_PERIOD`, `subjectId`, `version`, `kind` `ARCHIVE | EMAIL | SMS`, `reference`, `jobRunId` → `JobRun`, `deliveredAt` — archive and mail are separate async steps and separate rows; later notification subjects add an enum value). Migration `0015` bundles it with `User.notificationChannels`; its data line inserts an `EMAIL` v1 delivery for periods closed before 2026-09-17 (convergent: `NOT EXISTS`), so the first run archives their CSVs; mail starts with the next closed period. `useBilling().decideBillingSideEffects({version, archivedVersion, notifiedVersion})` (pure; the delivered versions are `deliveredVersions(rows)` from `useDeliveryValidation`, rows from `fetchDeliveries` / `recordDelivery` in `financesRepository.ts`, next to `bumpBillingPeriodVersion`) redoes what is behind; `notifyBillingPeriod` (also behind the re-send endpoint) records the `EMAIL` delivery on `queued`. R2 keeps one object per version (`…-v<n>.csv`), the attachment filename carries the version, and a later version is mailed as `BILLING_PERIOD_UPDATED` (subject `opdatering v<n>`, text names the version it replaces). Response: `results[]` (what this run billed) + `periods[]` (`{version, csvUploaded, emailSent, archive?, notification?}` for every closed period); the admin job-run line, cards and toast render both through `formatMonthlyBillingStats` (`CSV uploadet`, `Mails sendt`, `Afventer`, user 2026-09-18). Billing success depends on neither side effect. `server/tasks/monthly-billing.ts` and `server/routes/api/admin/maintenance/monthly.post.ts` pass `env.SENDER`, `env.ARCHIVE`, `getNotificationConfig()`.
- Tests: `tests/component/utils/billingArchive.unit.spec.ts` (versioned key, put, degraded, put failure); `useBilling.nuxt.spec.ts` (`decideBillingSideEffects`: first run / done / changed / archive done, mail pending / mail done, archive pending / migrated); `maintenance.e2e.spec.ts` asserts `periods[]` `csvUploaded` + `emailSent` on the first close and that a re-run redoes nothing, under `nuxt dev` (miniflare R2 + queue sink).
...
| `runMonthlyBilling` convergence | unit on the decision rule; existing maintenance API spec extended (`periods[]` end state, a second run reports the same state) | `tests/component/composables/useBilling.nuxt.spec.ts`, `tests/e2e/api/serial/admin/maintenance.e2e.spec.ts` |
...
npx wrangler r2 object get theslope-archive-dev/billing/2026-08/pbs-opgoerelse-2026-08-v1.csv --file test-results/pbs-2026-08-v1.csv   # after a dev monthly run
```
...
- Decision: R2 via a binding passed into the service (`archive?: R2Bucket`), ASCII deterministic keys per period version (`billing/YYYY-MM/…-v<n>.csv`, one object per version — ADR-015 convergence on the period's `version`), `customMetadata` with the human filename + version + jobRunId, result surfaced as an operation-result type (ADR-009), degraded mode logged and returned — never thrown.
