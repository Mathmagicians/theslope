...
- **`runMonthlyBilling(d1Client, triggeredBy, {queue, archive, notifications})`** (`server/utils/monthlyBillingService.ts`): after `generateBilling`, **every closed period converges** (user, 2026-09-17 — idempotency is the end state: transactions billed, CSV in R2, accountant mailed): `BillingPeriodSummary` carries `version` (domain: bumped by generation when catch-up changes a period); the delivery facts live in the generic append-only **`Delivery`** table (user, 2026-09-17: `subjectType` `BILLING_PERIOD`, `subjectId`, `version`, `kind` `ARCHIVE | EMAIL | SMS`, `reference`, `jobRunId` → `JobRun`, `deliveredAt` — archive and mail are separate async steps and separate rows; later notification subjects add an enum value). Migration `0015` bundles it with `User.notificationChannels`; its data line inserts an `EMAIL` v1 delivery for periods closed before 2026-09-17 (convergent: `NOT EXISTS`), so the first run archives their CSVs; mail starts with the next closed period. `decideBillingSideEffects({version, archivedVersion, notifiedVersion})` (pure, `server/utils/billingSideEffects.ts`; the delivered versions are `deliveredVersions(rows)` from `useDeliveryValidation`) redoes what is behind; `notifyBillingPeriod` (also behind the re-send endpoint) records the `EMAIL` delivery on `queued`. R2 keeps one object per version (`…-v<n>.csv`), the attachment filename carries the version, and a later version is mailed as `BILLING_PERIOD_UPDATED` (subject `opdatering v<n>`, text names the version it replaces). Response: `results[]` (what this run billed) + `periods[]` (`{version, csvUploaded, emailSent, archive?, notification?}` for every closed period). Billing success depends on neither side effect. `server/tasks/monthly-billing.ts` and `server/routes/api/admin/maintenance/monthly.post.ts` pass `env.SENDER`, `env.ARCHIVE`, `getNotificationConfig()`.
- Tests: `tests/component/utils/billingArchive.unit.spec.ts` (versioned key, put, degraded, put failure); `tests/component/utils/billingSideEffects.unit.spec.ts` (first run / done / changed / archive done, mail pending / mail done, archive pending / migrated); `maintenance.e2e.spec.ts` asserts `periods[]` `csvUploaded` + `emailSent` on the first close and that a re-run redoes nothing, under `nuxt dev` (miniflare R2 + queue sink).
...
| `runMonthlyBilling` convergence | unit on the decision rule; existing maintenance API spec extended (`periods[]` end state, a second run reports the same state) | `tests/component/utils/billingSideEffects.unit.spec.ts`, `tests/e2e/api/serial/admin/maintenance.e2e.spec.ts` |
