...
**Deletion safety is schema-designed, not caller-guarded** (ADR-005 + ADR-011): `Allergy`/`Order`/`CookingTeamAssignment` CASCADE; billing survives via `Transaction.orderSnapshot` (SET NULL); audit survives via `OrderHistory`'s denormalized `inhabitantId`/`dinnerEventId`/`seasonId`. Business-level chef-loss handling (Heynabo event delete + `CHEF_LOSS_DINNER_UPDATES` + allergen clear) lives in ONE shared server routine — callers MUST NOT duplicate guard logic.
