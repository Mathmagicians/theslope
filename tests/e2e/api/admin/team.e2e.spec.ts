...
let TEST_SEASON_ID: number // Will be set after creating the season

// Test team data (will be updated after season creation)
const newTeam = {
  get seasonId() { return TEST_SEASON_ID },
  name: testTeamName
}
...
let createdTeamId: number | undefined = undefined
...
test.describe('Admin Teams API', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    const { rawSeason, serializedSeason } = createTestSeason(`TeamTest-${testSalt}`, serializeSeason)
    const seasonResponse = await context.request.put(ADMIN_SEASON_ENDPOINT, {
      headers: headers,
      data: serializedSeason
    })

    if (seasonResponse.status() !== 201) {
      const errorBody = await seasonResponse.json()
      console.error('Season creation failed:', seasonResponse.status(), JSON.stringify(errorBody, null, 2))
      console.error('Season data sent:', JSON.stringify(rawSeason, null, 2))
    }

    expect(seasonResponse.status()).toBe(201)
    const createdSeason = await seasonResponse.json()
    TEST_SEASON_ID = createdSeason.id

    console.log(`Created test season ${createdSeason.shortName} with ID ${TEST_SEASON_ID}`)
  })

  test.describe('Admin Team Endpoints CRUD Operations', () => {

    test(`PUT ${ADMIN_TEAM_ENDPOINT} should create a new team`, async ({ browser }) => {
      const context = await browser.newContext({
        storageState: adminFile
      })
      ...
      expect(responseBody.seasonId).toBe(TEST_SEASON_ID)
    ...
    test(`GET ${ADMIN_TEAM_ENDPOINT}?seasonId=X should filter teams by season`, async ({ browser }) => {
      ...
      const team1 = { seasonId: TEST_SEASON_ID, name: `${testTeamName}-Season1` }
      const team2 = { seasonId: TEST_SEASON_ID + 1, name: `${testTeamName}-Season2` }
      ...
      createdTeamIds.push(createdTeam1.id, createdTeam2.id)

      // Filter teams by first season
      const filteredResponse = await context.request.get(`${ADMIN_TEAM_ENDPOINT}?seasonId=${TEST_SEASON_ID}`)
    ...
    test(`GET ${ADMIN_TEAM_ENDPOINT}/[id] should get specific team details`, async ({ browser }) => {
      ...
      expect(teamDetails.seasonId).toBe(TEST_SEASON_ID)
    ...
    test(`DELETE ${ADMIN_TEAM_ENDPOINT}/[id] should delete team`, async ({ browser }) => {
      const context = await browser.newContext({
        storageState: adminFile
      })
  ...
  test.describe('Team Member Management', () => {
    ...
    test(`PUT ${ADMIN_TEAM_ENDPOINT}/[id]/members should add team members`, async ({ browser }) => {
      const context = await browser.newContext({
        storageState: adminFile
      })
    ...
    test(`DELETE ${ADMIN_TEAM_ENDPOINT}/[id]/members/[memberId] should remove team member`, async ({ browser }) => {
      const context = await browser.newContext({
        storageState: adminFile
      })
  ...
  test.afterAll(async ({ browser }) => {
    ...
    if (TEST_SEASON_ID) {
      try {
        await context.request.delete(`${ADMIN_SEASON_ENDPOINT}/${TEST_SEASON_ID}`)
        console.log(`Cleaned up test season with ID ${TEST_SEASON_ID}`)
      } catch (error) {
        console.log(`Failed to cleanup test season ${TEST_SEASON_ID}:`, error)
      }
    }
  })
})
