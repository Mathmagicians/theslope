...
test.describe('Household members display', () => {
    ...
    test('GIVEN household member WHEN editing weekday preference THEN change is persisted to database', async ({page, browser}) => {
        ...
        await pollUntil(
            async () => await page.getByTestId('household-members').isVisible(),
            (isVisible) => isVisible,
            10
        )

        const editButton = page.getByTestId('household-members-edit-toggle')
        await editButton.click()

        await pollUntil(
            async () => {
                const button = page.locator(`button[name="inhabitant-${donaldId}-preferences-MONDAY-TAKEAWAY"]`)
        ...
        await doScreenshot(page, 'household-members-edit-mode')

        const mondayTakeawayButton = page.locator(`button[name="inhabitant-${donaldId}-preferences-MONDAY-TAKEAWAY"]`)
        await mondayTakeawayButton.click()

        await page.waitForResponse(
            (response) => response.url().includes(`/api/admin/household/inhabitants/${donaldId}`) && response.status() === 200,
            {timeout: 10000}
        )

        const household = await HouseholdFactory.getHouseholdById(context, householdId)
